# Type flags for PSO type
abstract type AbstractPSOType end
struct SerialPSO <: AbstractPSOType end
struct ThreadedPSO <: AbstractPSOType end
struct PolyesterPSO <: AbstractPSOType end
...
function take_integration_step!(ps, mee, mass, time, L0, Lf, sa_constrained, cache::Nothing)
    ...
    fn = ODEFunction{false,SciMLBase.FullSpecialize}((u,p,L) -> qLawEOMsSundmanTransformedZOH(u,p,L,ps.meePs))
    prb = ODEProblem(fn, x0, (L0,Lf), pt)
    sol = solve(
        prb, ps.desolver; 
        reltol = ps.reltol,
        abstol = ps.abstol,
...
get_pso_constructor(pso_type::Type{SerialPSO}) = GlobalOptimization.SerialPSO
get_pso_constructor(pso_type::Type{ThreadedPSO}) = GlobalOptimization.ThreadedPSO
get_pso_constructor(pso_type::Type{PolyesterPSO}) = GlobalOptimization.PolyesterPSO
...
function generate_qlaw_transfer(
    ...
    pso_constructor = get_pso_constructor(pso_type)
    pso = pso_constructor(
        prob; 
        max_time = max_time, 
