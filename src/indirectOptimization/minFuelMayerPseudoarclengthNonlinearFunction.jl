...
function minFuelMayerFixedPointNonlinearFunction!(F, x, xi, dc, Δs, x1, κ1, mee0, meef_interp, pTuple)
    # Compute nonlinear function
    @views minFuelMayerNonlinearFunctionWithInterp!(F[1:6], x[1:6], κ1, mee0, meef_interp, pTuple)

    # Modify function results for fixed point interation
    ...
    # Final term for step constraint
    F[7]    = -Δs
    @inbounds for i in 1:7
        F[7] += (x[i] - xi[i])*dc[i]
    end
...
function minFuelMayerFixedPointNonlinearFunctionWithJacobian!(F,J,x,xi,dc,Δs,x1,κ1,mee0,meef_interp,pTuple,result,mem,config)
    if J === nothing && F !== nothing
        minFuelMayerFixedPointNonlinearFunction!(F, x, xi, dc, Δs, x1, κ1, mee0, meef_interp, pTuple)
    elseif J !== nothing
        result = ForwardDiff.jacobian!(result,
                    (F,x) -> minFuelMayerFixedPointNonlinearFunction!(F, x, xi, dc, Δs, x1, κ1, mee0, meef_interp, pTuple),
                    mem, x, config, Val{false}())
        J .= DiffResults.jacobian(result)
        if F !== nothing
            F .= DiffResults.value(result)
...
function minFuelMayerNonlinearFunctionWithInterp!(F, x, κ1, mee0, meef_interp, pTuple)
    # Grab the interpolated state for the current value of κ 
    val = getFinalStateAndTime(meef_interp, κ1)
